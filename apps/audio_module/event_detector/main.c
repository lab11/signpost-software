...
static kiss_fft_scalar frame_buf[BUF_LEN];

//The ADC callback get called when the buffer is full
static void adc_callback (
        uint8_t channel __attribute__((unused)),
        uint32_t number_of_samples __attribute ((unused)),
        uint16_t* buffer __attribute__((unused)),
        void* ud __attribute__((unused))) {

    static size_t frameIdx = 0;

    //make this rentrant - don't copy data into frame while processing it
    if(more_data != true) {
        memcpy(frame_buf+frameIdx, buffer, INC_LEN);
...
int main (void) {
    ...
    static uint16_t adc_buf[INC_LEN];
    static uint16_t adc_buf2[INC_LEN];
    //static global should now be zero
    ...
    //setup the ADC
    //Note that we could completely remove the need for adc_buf and the memcpy
    //by passing in pointers to two locations in the frame buffer to the adc
    //But this would only work when BUF_LEN/INC_LEN == 2 (as it does now)
    adc_set_buffer(adc_buf, INC_LEN);
    adc_set_double_buffer(adc_buf2, INC_LEN);
    adc_set_continuous_buffered_sample_callback(adc_callback, NULL);
    adc_continuous_buffered_sample(2, 44100);

    while (true) {
        // read data from ADC
        // This was replaced by the callback
